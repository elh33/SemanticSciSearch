...
const SearchResults: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* En-tête avec fond bleu - GRANDE BARRE */}
      <div style={{ backgroundColor: '#3883c5ff' }} className="sticky top-0 z-10 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-10">
          <div className="flex items-center justify-between gap-6">
            {/* Espace vide à gauche pour centrer la barre */}
            <div className="flex-1"></div>
            
            {/* Barre de recherche au centre - TRÈS GRANDE et entourée en jaune */}
            <form onSubmit={handleSubmit} className="flex-1 max-w-4xl">
              <div className="flex shadow-2xl" style={{ border: '5px solid #fbbf24', borderRadius: '30px', backgroundColor: 'white' }}>
                <input
                  type="text"
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder="Rechercher des articles scientifiques..."
                  className="flex-grow px-10 py-5 text-xl outline-none"
                  style={{ borderRadius: '12px 0 0 12px' }}
                />
                <button
                  type="submit"
                  className="px-10 py-5 text-white text-xl font-bold hover:opacity-90 transition-opacity"
                  style={{ backgroundColor: '#2563eb', borderRadius: '0 12px 12px 0' }}
                >
                  🔍
                </button>
              </div>
      ...
      <footer className="mt-12 border-t border-gray-200 bg-gray-50">
        <div className="max-w-5xl mx-auto px-4 py-6 text-center text-sm text-gray-500">
          SemanticSciSearch - Rherche sémantique d'articles scientifiques en IA et Cardiologie
        </div>
      </footer>
    </div>
  );
