...
const SearchResults: React.FC = () => {
  ...
  // Recherche initiale au chargement (avec query vide)
  useEffect(() => {
    handleSearch('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    handleSearch(); // Lance la recherche uniquement lors de la soumission du formulaire
  };
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '24px 16px' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', gap: '8px', width: '100%', maxWidth: '768px' }}>
            ...
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)} // Met juste à jour l'état, ne lance pas la recherche
